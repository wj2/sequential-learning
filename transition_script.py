...
shape_sequence = (
    "A2", "A3", "A4", "A3postA4", "A5", "A4postA5", "A3postA5", "A6", "A7", "A6postA7",
)
...
if __name__ == '__main__':
    ...
    if args.sequence_ind >= len(shape_sequence) - 1:
        raise IOError("ind {} is too high for list {}".format(
            args.shape_ind, shape_sequence
        ))
    else:
        s1 = shape_sequence[args.shape_ind]
        s2 = shape_sequence[args.shape_ind + 1]
    ...
    out = sla.compute_cross_shape_generalization(
        data_dict[s1], data_dict[s2], args.winsize, args.tbeg, args.tend, args.winstep,
    )
    ...
    slv.plot_cross_shape_generalization(
        *out, session_color=session_color, shape_color=shape_color, ax=ax,
    )
